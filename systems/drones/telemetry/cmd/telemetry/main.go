// Binary for the deliverydron telemetry component (aggregate motors + cargo state).
package main
...
func main() {
	cfg := config.FromEnv()
	log.Printf("[%s] telemetry broker_type=%s topic=%s", cfg.ComponentID, cfg.BrokerType, cfg.ComponentTopic)
