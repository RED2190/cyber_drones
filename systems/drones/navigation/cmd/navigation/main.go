// Binary for the deliverydron navigation component (nav state, get_state).
package main
...
func main() {
	cfg := config.FromEnv()
	log.Printf("[%s] navigation broker_type=%s topic=%s", cfg.ComponentID, cfg.BrokerType, cfg.ComponentTopic)
