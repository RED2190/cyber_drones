...
с docker-compose компонентов в единый .generated/docker-compose.yml + .generated/.env.

Ожидаемая схема — как в sbd-drones-economics:
  <корень монорепозитория>/
    docker/                 # брокер
    scripts/                # общий prepare (можно не дублировать)
    config/
    systems/agrodron/       # этот проект: Makefile, docker-compose.yml, src/<компонент>/.env

Корень монорепозитория определяется по наличию docker/docker-compose.yml (подъём вверх от
этого файла). Скрипт может лежать в корне монорепозитория (scripts/prepare_system.py) или
внутри системы: systems/agrodron/scripts/prepare_system.py.

Из корня монорепозитория:

    python systems/agrodron/scripts/prepare_system.py systems/agrodron

или, если скрипт скопирован в scripts/:

    python scripts/prepare_system.py systems/agrodron

Автономный клон только с каталогом дронов (рядом docker/ и этот скрипт в scripts/):
...
def resolve_monorepo_root() -> Path:
    """Корень дерева с docker/docker-compose.yml (монорепозиторий или автономный клон дронов)."""
...
def prepare_system(system_dir: str) -> None:
    ...
    # Discover components: components/ (старый layout) или src/ (cyber_drons)
    components_dir = (
    ...
    with open(compose_out, "w") as f:
        f.write(
            ...
            "#   python systems/agrodron/scripts/prepare_system.py "
            f"{system_dir_arg}\n"
        )
...
if __name__ == "__main__":
    if len(sys.argv) != 2:
        print(
            "Usage: python .../prepare_system.py <system_dir>\n"
            "Monorepo: python systems/agrodron/scripts/prepare_system.py systems/agrodron\n"
            "Standalone: python scripts/prepare_system.py .",
            file=sys.stderr,
        )
        sys.exit(1)
