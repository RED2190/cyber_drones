...
| `request_departure` | `{"mission_id": str}` | `{"approved": true, "battery": float, "port_id": str, "drone_id": str, "port_coordinates": {"lat": str, "lon": str}, "from": "drone_manager"}` или `{"error": str, "from": "drone_manager"}` | Разрешить вылет, освободить порт и отправить событие старта взлета в SITL |
...
- `drone_manager` - отправляет команды в дроновый контур и обновляет состояние миссии/борта.
...
- отправить команду загрузки миссии в дроновый контур через `drone_manager`;
...
Синхронный ответ не предусмотрен. Подтверждение нужно отслеживать по внутреннему состоянию миссии и дрона.
...
Во внутреннем сообщении между `orchestrator` и `drone_manager` WPL передается в поле `wpl`.
...
- собрать `.generated/docker-compose.yml` для `external/cyber_drons/agrodron`;
- поднять broker, GCS, DronePort и AgroDron перед публикацией UI;
